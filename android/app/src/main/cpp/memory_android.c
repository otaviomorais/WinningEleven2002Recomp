    ...
    if (bios_path && bios_path[0]) {
        ...
        if (f) {
            ...
            if (n == BIOS_ROM_SIZE) {
                ...
                fprintf(stderr, "WARN: BIOS file %s is %zu bytes (expected %d)\n",
                        bios_path, n, BIOS_ROM_SIZE);
            }
        } else {
            fprintf(stderr, "WARN: cannot open BIOS file: %s\n", bios_path);
        }
    }
    if (!loaded) {
        fprintf(stderr, "INFO: Loading embedded OpenBIOS (%d bytes)\n", BIOS_ROM_SIZE);
        memcpy(bios_rom, g_embedded_openbios, BIOS_ROM_SIZE);
        loaded = 1;
    }
    s_bios_checksum = 0;
